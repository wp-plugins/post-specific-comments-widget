		...
		if ( $comments ) {
			foreach ( (array) $comments as $comment) {
				...
				if ($instance['comment_format'] == "author-post") {
					$output .=  '<li class="recentcomments">' . printf(__('%1$s on %2$s', 'pscw'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '" class="' . $classes . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
				}

				if ($instance['comment_format'] == "author-excerpt") {
					$output .= '<li class="recentcomments">' . sprintf(__( '<span class="recentcommentsauthor">%1$s</span> said %2$s', 'pscw' ), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . $aRecentCommentTxt . '</a>') . '</li>';	
				}
			}
 		}
		$output .= '</ul>';
